...
function readTyreWear(img, scale) {
  return new Promise(resolve => {
    ...
    for (const key of TYRE_KEYS) {
      ...
      // Count white pixels (remaining wear) and red pixels (consumed wear)
      // separately. Anti-aliased edge pixels and other colours are excluded
      // from both counts so they cannot skew the ratio.
      let white = 0;
      let red = 0;
      ...
      const barTotal = white + red;
      const pct = barTotal > 0 ? Math.min(100, (white / barTotal) * 100) : 0;
      readings[key] = parseFloat(pct.toFixed(1));
      debugLines.push(`${key.toUpperCase()}: ${white} white + ${red} red = ${barTotal} bar pixels, ${pct.toFixed(1)}% remaining`);
    }
